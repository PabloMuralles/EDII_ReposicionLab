...
namespace LabReposicion.LZW.Compresion
{
    public class AlgoritmoDescompresion
    {
        ...
        public AlgoritmoDescompresion(IFormFile Archivo, string nombre)
        {
            ...
            DatosDescompresion datos = new DatosDescompresion();

            datos.TamañoComprimido = Convert.ToInt32(fileHistorial.Length);
            datos.NombreOrignalComprido = fileHistorial.FileName;
            datos.Ruta = Path.Combine(Environment.CurrentDirectory, "decompressLZW", $"{Nombre}.txt");

            HistorialCompresion.Instance.ArchivosDescomprimidosPils.Add(datos);
